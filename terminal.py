#!/usr/bin/env python3
"""
Python Terminal Emulator for Linux Mint
...
class TerminalManager:
    """Manages terminal operations and custom commands"""
    ...
    def _validate_system() -> None:
        """Validate that the system is Linux Mint"""
        if sys.platform != 'linux':
            print(f"{TerminalColor.RED}Error: This terminal is compatible only with Linux Mint.{TerminalColor.END}")
            print(f"Current system: {platform.system()}")
            sys.exit(1)
        
        # Check if running on Linux Mint specifically
        try:
            with open('/etc/os-release', 'r') as f:
                os_info = f.read().lower()
                if 'mint' not in os_info and 'ubuntu' not in os_info:
                    print(f"{TerminalColor.YELLOW}Warning: This terminal is optimized for Linux Mint.{TerminalColor.END}")
        except FileNotFoundError:
            print(f"{TerminalColor.YELLOW}Warning: Could not verify Linux Mint distribution.{TerminalColor.END}")
    ...
    def cmd_ls(self, args: List[str]) -> None:
        ...
        try:
            ...
            if args and args[0].startswith('-'):
                ...
                ls_cmd.append('-1')
                target_dir = os.path.join(self.current_dir, args[0]) if args else self.current_dir
            
            result = subprocess.run(ls_cmd + [target_dir], capture_output=True, text=True)
            if result.stdout:
                print(result.stdout)
            if result.stderr:
                print(f"{TerminalColor.RED}{result.stderr}{TerminalColor.END}")
        except Exception as e:
            print(f"{TerminalColor.RED}Error: {e}{TerminalColor.END}")
    ...
    def cmd_help(self, args: List[str]) -> None:
        ...
        print(f"\n{TerminalColor.BOLD}{TerminalColor.CYAN}Available Commands:{TerminalColor.END}\n")
        commands_help = {
            ...
            'savecmd <name> <command>': 'Save a named command',
    ...
    def cmd_menu(self, args: List[str]) -> None:
        ...
        # If user provided a command name directly, execute it.
    ...
    def _execute_system_command(self, cmd: str, args: List[str]) -> None:
        ...
        try:
            full_cmd = [cmd] + args
            result = subprocess.run(full_cmd, cwd=self.current_dir, capture_output=True, text=True)
            if result.stdout:
    ...
    def process_command(self, input_str: str) -> None:
        ...
        # Check for aliases
        if input_str.split()[0] in self.aliases:
            input_str = self.aliases[input_str.split()[0]] + ' ' + ' '.join(input_str.split()[1:])
            input_str = input_str.strip()
    ...
    def prompt(self) -> str:
        ...
        hostname = os.getenv('HOSTNAME', 'linuxmint')
        short_dir = self.current_dir.replace(os.path.expanduser('~'), '~')
        return f"{TerminalColor.GREEN}{user}@{hostname}{TerminalColor.END}:{TerminalColor.BLUE}{short_dir}{TerminalColor.END}$ "
    
    def run(self) -> None:
        """Start the terminal"""
        print(f"\n{TerminalColor.HEADER}{TerminalColor.BOLD}Python Terminal for Linux Mint{TerminalColor.END}")
        ...
        try:
            while True:
                try:
                    ...
                    print(f"\n{TerminalColor.YELLOW}Interrupted by user{TerminalColor.END}")
                    break
        except EOFError:
